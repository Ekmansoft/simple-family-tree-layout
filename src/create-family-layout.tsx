...
import { layoutNextPoint, defaultFamilySize, defaultProfileOffsetHorizontal, defaultChildOffsetVertical, defaultParentOffsetVertical, defaultProfileSize } from './index';
import { Point } from './point';
import { Rectangle } from './rectangle';
import { FamilyLink, ProfileLink, TreeBackend } from 'simple-family-tree-model';

export function createFamilyLayout(tree: TreeBackend, startProfile: ProfileLink) : RootLayout {
    ...
    if (profile !== undefined) {
      ...
      console.log("show profile " + startProfileId + " child in " +
        profile.childInFamilies.getLinks().length + " families, parent in " +
        profile.parentInFamilies.getLinks().length + " families ");
      ...
      parentFamilies.forEach((element) => {
        ...
        if (parentFamily !== undefined) {
          ...
          if (!layout.families.has(familyId)) {
            console.log("From parent " + startProfileId + "List family: " + familyId);
            console.log("show family " + familyId + " children:" +
              parentFamily.children.getLinks().length + " parents:" +
              parentFamily.parents.getLinks().length);
            const familyLayout = new FamilyLayout(familyId, new Rectangle(layoutNextPoint, defaultFamilySize));
            ...
            spouses.forEach((element) => {
              ...
              if (parent !== undefined) {
                ...
                if (!layout.profiles.has(profileId)) {
                  console.log("From parent " + startProfileId + " family " + familyId + " add parent " + profileId);
                  const parentPoint = new Point(familyLayout.rect.topLeft.x - 20 + (familyLayout.parents.length * defaultProfileOffsetHorizontal), familyLayout.rect.topLeft.y+defaultParentOffsetVertical);
                  const placement: Rectangle = new Rectangle(parentPoint, defaultProfileSize);
                  layout.profiles.set(profileId, familyId);
                  familyLayout.profiles.set(profileId, new ProfileInfo(placement, parent));
            ...
            children.forEach((element) => {
              ...
              if (child !== undefined) {
                ...
                if (!layout.profiles.has(profileId)) {
                  console.log("From parent " + startProfileId + ", family " + familyId + " add child " + profileId);
                  const childPoint = new Point(familyLayout.rect.topLeft.x - 20 + (familyLayout.children.length * defaultProfileOffsetHorizontal), familyLayout.rect.topLeft.y+defaultChildOffsetVertical);
                  const placement: Rectangle = new Rectangle(childPoint, defaultProfileSize);
                  layout.profiles.set(profileId, familyId);
                  familyLayout.profiles.set(profileId, new ProfileInfo(placement, child));
          ...
          if (!layout.families.has(familyId)) {
            ...
            children.forEach((element) => {
              ...
              if (child !== undefined) {
                ...
                if (!layout.profiles.has(profileId)) {
                  const childPoint = new Point(familyLayout.rect.topLeft.x - 20 + (familyLayout.children.length * defaultProfileOffsetHorizontal), familyLayout.rect.topLeft.y+defaultChildOffsetVertical);
                  const placement: Rectangle = new Rectangle(childPoint, defaultFamilySize);
