...
export class FamilyLayout {
  ...
  getOuterBounds = (): Rectangle => {
    ...
    for (const profileLayout of this.profiles.values()) {
      ...
      if (
        outerRect.getBottomRight().x < profileLayout.layout.getBottomRight().x
      ) {
        outerRect.size.x =
          profileLayout.layout.getBottomRight().x - outerRect.topLeft.x;
      }
      if (
        outerRect.getBottomRight().y < profileLayout.layout.getBottomRight().y
      ) {
        outerRect.size.y =
